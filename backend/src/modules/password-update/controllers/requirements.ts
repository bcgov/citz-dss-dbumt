...
import { addPasswordRequirement, findPasswordRequirementByName, getAllPasswordRequirements } from "@/modules/password-update/services"
...
export const addRequirement = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR).send(err_message)
  }
...
export const findReqByName = async (req: Request, res: Response) => {
  ...
  if (!foundReq) {
    return res.status(HTTP_STATUS_CODES.NOT_FOUND).send("Requirement not found")
  }
...
export const getAllReq = async (req: Request, res: Response) => {
  ...
  if (!allReq) {
    return res.status(HTTP_STATUS_CODES.NOT_FOUND).send("No Requirements found")
  }
